...
public class Ma4_9 : MonoBehaviour
{
    ...
    void Next_Stage()
    {
        ...
        if (Input.GetKey("space"))
        {
            ...
            if (End.grade > PlayerPrefs.GetInt("c9", 0))   // 점수 저장용
            {
                PlayerPrefs.SetInt("c9", End.grade);
            }
            Debug.Log(PlayerPrefs.GetInt("c9"));
    ...
    void Update()
    {
        ...
        {
            if (PlayerPrefs.GetInt("Score") < 30)     //최고스테이지가 이번스테이지가 아닐경우는  if문을 들어가지 않는다
            {
                ClearCount.score = 30;    // if속으로 들어오면 score값 변화
